...
export let createPlansLanguageReferat = async (
  ...
  let queryJson = {
    ...
    input_text: `Create a layout with ${pages} sections for the topic ${name}. Each section should provide a concise plan for structuring a research paper or project on the given topic. Each section should include an introduction, body, and conclusion in both ${language} and English. Keep descriptions brief, around 15 to 20 words per section in each language. The plans must contain only textual information with clear and concise instructions for independent work. Ensure the structure is correct and the plans are professional. Stick to the format without additional or unrelated information.`,
    output_format: "json",
    json_structure: {
      slides: {
        plans: [
